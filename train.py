...
def create_time_series_data(raw_df: pd.DataFrame) -> pd.DataFrame:
    np.random.seed(42)
    # Synthesize daily series informed by dataset aggregates
...
def train_and_evaluate(months_holdout: int = 12, order=(1, 1, 1), seasonal_order=(1, 1, 1, 12)):
    ...
    monthly = (
        ts_data.groupby(["year", "month"])  # monthly aggregation
        .agg({"bookings": "mean"})
        .reset_index()
    )
    ...
    metrics = {
        ...
        "rmse": rmse,
        "mape": mape,
        "holdout_months": months_holdout,
    }

    print("\n✅ Evaluation on holdout:")
    print(json.dumps(metrics, indent=2))

    # Metadata used by the API (defaults if not derivable)
    ...
    print("\n💾 Saved model to airbnb_forecast_model.pkl")
    return metrics
